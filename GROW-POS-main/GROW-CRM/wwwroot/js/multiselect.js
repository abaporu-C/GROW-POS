...
use the sender param for the DDL from which the user is multi-selecting
use the receiver param for the DDL that gets the options*/
function switchItemOptions(event, senderDDL, receiverDDL) {
    //find all selected option tags - selectedOptions becomes a nodelist 
    let senderID = senderDDL.id;
    let selectedOptions = document.querySelectorAll(`#${senderID} option:checked`);
    event.preventDefault();
...
//create closures so that we can access the event & the 2 parameters
let addItemOptions = (event) => switchItemOptions(event, DDLforAvailItem, DDLforChosenItem);
let removeItemOptions = (event) => switchItemOptions(event, DDLforChosenItem, DDLforAvailItem);
//assign the closures as the event handlers for each button
document.getElementById("btnItemLeft").addEventListener("click", addItemOptions);
document.getElementById("btnItemRight").addEventListener("click", removeItemOptions);

document.getElementById("btnSubmit").addEventListener("click", function () {
    DDLforChosenItem.childNodes.forEach(opt => opt.selected = "selected");
})